 ...
 * Instead, change the Metrics/Action.js file.
 *
 * @module Metrics
 */
 ...
 * @param {String|Buffer} [fields.canonicalActionId] defaults to ""
 */
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Metrics', 'indexes', 'Action'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{{dbname}}.')+'{{prefix}}action');
	} else {
		var conn = Db.getConnection('Metrics');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'action';
 ...
 * @type string
 */
Base.prototype.className = "Metrics_Action";
...
return [["varbinary","2083","",false],true,"MUL",null];
};
 ...
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_canonicalActionId = function (value) {
		if (value == null) {
			value='';
		}
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number" && !(value instanceof Buffer))
			throw new Error('Must pass a String or Buffer to '+this.table()+".canonicalActionId");
		if (typeof value === "string" && value.length > 63)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".canonicalActionId");
		return value;
};
	 ...
	 * @return {integer}
	 */
Base.prototype.maxSize_canonicalActionId = function () {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_canonicalActionId = function () {
