 ...
 * Instead, change the Metrics_Conversion.php file.
 *
 * @module Metrics
 */
/**
 * Base class representing 'Conversion' rows in the 'Metrics' database
 * @class Base_Metrics_Conversion
 * @extends Db_Row
 *
 * @param {array} [$fields=array()] The fields values to initialize table row as 
 * an associative array of $column => $value pairs
 * @param {string} [$fields.traitId] defaults to ""
 * @param {string} [$fields.actionId] defaults to ""
 * @param {integer} [$fields.visitsTotal] defaults to 0
 * @param {float} [$fields.sinceTracker] defaults to 0
 * @param {float} [$fields.sinceVisit] defaults to 0
 * @param {string|Db_Expression} [$fields.insertedTime] defaults to new Db_Expression("CURRENT_TIMESTAMP")
 * @param {string|Db_Expression} [$fields.updatedTime] defaults to null
 */
abstract class Base_Metrics_Conversion extends Db_Row
{
	/**
	 * @property $traitId
	 * @type string
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Metrics', 'indexes', 'Conversion', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'conversion');
		} else {
			$conn = Db::getConnection('Metrics');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'conversion';
  			if (!$with_db_name)
  				return $table_name;
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Metrics_Conversion'))
		);
	}
	 ...
	 */
	function beforeSet_traitId($value)
	{
		if (!isset($value)) {
			$value='';
		}
	 ...
	 */
	static function column_traitId()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_actionId($value)
	{
		if (!isset($value)) {
			$value='';
		}
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_actionId()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 */
	function beforeSet_visitsTotal($value)
	{
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('visitsTotal', $value);
		}
		if (!is_numeric($value) or floor($value) != $value)
			throw new Exception('Non-integer value being assigned to '.$this->getTable().".visitsTotal");
		$value = intval($value);
		if ($value < -2147483648 or $value > 2147483647) {
			$json = json_encode($value);
			throw new Exception("Out-of-range value $json being assigned to ".$this->getTable().".visitsTotal");
		}
		return array('visitsTotal', $value);			
	}

	/**
	 * @method maxSize_visitsTotal
	 * Returns the maximum integer that can be assigned to the visitsTotal field
	 * @return {integer}
	 */
	function maxSize_visitsTotal()
	{
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_visitsTotal()
	{
...
return array (
  ...
  3 => NULL,
	...
	function beforeSet_sinceTracker($value)
	{
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('sinceTracker', $value);
		}
		if (!is_numeric($value))
			throw new Exception('Non-numeric value being assigned to '.$this->getTable().".sinceTracker");
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_sinceTracker()
	{
...
return array (
  ...
  3 => NULL,
	...
	function beforeSet_sinceVisit($value)
	{
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('sinceVisit', $value);
		}
		if (!is_numeric($value))
			throw new Exception('Non-numeric value being assigned to '.$this->getTable().".sinceVisit");
		$value = floatval($value);
		return array('sinceVisit', $value);			
	}
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_sinceVisit()
	{
...
return array (
  ...
  3 => NULL,
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('traitId','actionId') as $name) {
				if (!isset($value[$name])) {
	...
	{
		$field_names = array('traitId', 'actionId', 'visitsTotal', 'sinceTracker', 'sinceVisit', 'insertedTime', 'updatedTime');
		$result = $field_names;
