 ...
 * Instead, change the Metrics_Trait.php file.
 *
 * @module Metrics
 */
/**
 * Base class representing 'Trait' rows in the 'Metrics' database
 * @class Base_Metrics_Trait
 * @extends Db_Row
 *
 ...
 */
abstract class Base_Metrics_Trait extends Db_Row
{
	/**
	 * @property $id
	 * @type string
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Metrics', 'indexes', 'Trait', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'trait');
		} else {
			$conn = Db::getConnection('Metrics');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'trait';
  			if (!$with_db_name)
  				return $table_name;
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Metrics_Trait'))
		);
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 */
	function beforeSet_sentTotal($value)
	{
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('sentTotal', $value);
		}
		if (!is_numeric($value) or floor($value) != $value)
			throw new Exception('Non-integer value being assigned to '.$this->getTable().".sentTotal");
		$value = intval($value);
		if ($value < -2147483648 or $value > 2147483647) {
			$json = json_encode($value);
			throw new Exception("Out-of-range value $json being assigned to ".$this->getTable().".sentTotal");
		}
		return array('sentTotal', $value);			
	}

	/**
	 * @method maxSize_sentTotal
	 * Returns the maximum integer that can be assigned to the sentTotal field
	 * @return {integer}
	 */
	function maxSize_sentTotal()
	{
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_sentTotal()
	{
...
return array (
  ...
  3 => NULL,
	...
	function beforeSave($value)
	{
		...
		if (!isset($this->fields["sentTotal"]) and !isset($value["sentTotal"])) {
			$this->sentTotal = $value["sentTotal"] = 0;
		}
	...
	{
		$field_names = array('id', 'name', 'content', 'sentTotal', 'visitsTotal', 'insertedTime', 'updatedTime');
		$result = $field_names;
