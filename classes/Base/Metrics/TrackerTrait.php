...
/**
 * Autogenerated base class representing tracker_trait rows
 * in the Metrics database.
 *
 * Don't change this file, since it can be overwritten.
 * Instead, change the Metrics_TrackerTrait.php file.
 *
 * @module Metrics
 */
/**
 * Base class representing 'TrackerTrait' rows in the 'Metrics' database
 * @class Base_Metrics_TrackerTrait
 * @extends Db_Row
 *
 ...
 */
abstract class Base_Metrics_TrackerTrait extends Db_Row
{
	/**
	 * @property $trackerId
	 * @type string
	 * @default ""
	 * the tracker to which the trait applies
	 */
	/**
	 * @property $traitId
	 * @type string
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Metrics', 'indexes', 'TrackerTrait', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'tracker_trait');
		} else {
			$conn = Db::getConnection('Metrics');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'tracker_trait';
  			if (!$with_db_name)
  				return $table_name;
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Metrics_TrackerTrait'))
		);
	}
...
return array (
  ...
  3 => NULL,
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('trackerId','traitId') as $name) {
				if (!isset($value[$name])) {
	...
	{
		$field_names = array('trackerId', 'traitId', 'insertedTime', 'updatedTime');
		$result = $field_names;
