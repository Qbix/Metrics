 ...
 * Instead, change the Metrics_Hit.php file.
 *
 * @module Metrics
 */
/**
 * Base class representing 'Hit' rows in the 'Metrics' database
 * @class Base_Metrics_Hit
 * @extends Db_Row
 *
 * @param {array} [$fields=array()] The fields values to initialize table row as 
 * an associative array of $column => $value pairs
 * @param {string} [$fields.visitId] defaults to ""
...
abstract class Base_Metrics_Hit extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Metrics', 'indexes', 'Hit', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'hit');
		} else {
			$conn = Db::getConnection('Metrics');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'hit';
  			if (!$with_db_name)
  				return $table_name;
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Metrics_Hit'))
		);
	}
	 ...
	 */
	function beforeSet_visitId($value)
	{
		if (!isset($value)) {
			$value='';
		}
	 ...
	 */
	static function column_visitId()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_fromActionId($value)
	{
		if (!isset($value)) {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_fromActionId()
	{
