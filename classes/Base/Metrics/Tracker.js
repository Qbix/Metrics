 ...
 * Instead, change the Metrics/Tracker.js file.
 *
 * @module Metrics
 */
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Metrics', 'indexes', 'Tracker'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{{dbname}}.')+'{{prefix}}tracker');
	} else {
		var conn = Db.getConnection('Metrics');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'tracker';
 ...
 * @type string
 */
Base.prototype.className = "Metrics_Tracker";
 ...
 */
Base.prototype.beforeSet_sentCount = function (value) {
		if (value instanceof Db.Expression) return value;
		value = Number(value);
		if (isNaN(value) || Math.floor(value) != value) 
			throw new Error('Non-integer value being assigned to '+this.table()+".sentCount");
		if (value < -2147483648 || value > 2147483647)
			throw new Error("Out-of-range value "+JSON.stringify(value)+" being assigned to "+this.table()+".sentCount");
		return value;
};
 ...
 * @return {integer}
 */
Base.prototype.maxSize_sentCount = function () {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_sentCount = function () {
 ...
 * @throws {Error} An exception is thrown if 'value' is not integer or does not fit in allowed range
 */
Base.prototype.beforeSet_visitsCount = function (value) {
		if (value instanceof Db.Expression) return value;
		value = Number(value);
		if (isNaN(value) || Math.floor(value) != value) 
			throw new Error('Non-integer value being assigned to '+this.table()+".visitsCount");
		if (value < -2147483648 || value > 2147483647)
			throw new Error("Out-of-range value "+JSON.stringify(value)+" being assigned to "+this.table()+".visitsCount");
		return value;
};
 ...
 * @return {integer}
 */
Base.prototype.maxSize_visitsCount = function () {
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_visitsCount = function () {
...
Base.prototype.beforeSave = function (value) {
	...
	if (this.fields["sentCount"] == undefined && value["sentCount"] == undefined) {
		this.fields["sentCount"] = value["sentCount"] = 0;
	}
	if (this.fields["visitsCount"] == undefined && value["visitsCount"] == undefined) {
		this.fields["visitsCount"] = value["visitsCount"] = 0;
	}
	return value;
};
